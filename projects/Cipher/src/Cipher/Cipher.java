	...
	private class engine implements ActionListener{
		...
		public void actionPerformed(ActionEvent e){
			...
			if(temp.equals("Exit")){
				...
				JOptionPane.showMessageDialog(Cipher.this,
		...
		public void parse(int z){
			...
			aPW = temp.toLowerCase().toCharArray();
			...
			if(z == 0){
				aMessage = leftA.getText().toLowerCase().toCharArray();
			}else{
				aMessage = rightA.getText().toLowerCase().toCharArray();
			}
			nMessage = new int[aMessage.length];
			for(int i = 0; i < nMessage.length; i++){
				int iTemp = (int)aMessage[i]-96;
				if(iTemp < 1 || iTemp > 26){
					iTemp = -64;
				}
				nMessage[i] = iTemp;
		...
		public String encode() {
			...
			while(mTally < nMessage.length){
				for(int i = 0; i < nPW.length; i++){
					...
					if(temp > 26 && nMessage[mTally] != -64){
						temp -= 26;
					}
					eMessage[mTally] = (char)(temp + 96);
					mTally++;
				}
			}
		...
		public String decode(){
			...
			while(mTally < nMessage.length){
				for(int i = 0; i < nPW.length; i++){
					...
					int temp = (nMessage[mTally] - nPW[i]);
					if(temp < 1 && nMessage[mTally] != -64){
						temp += 26;
					}
					aMessage[mTally] = (char)(temp + 96);
					mTally++;
				}
			}
